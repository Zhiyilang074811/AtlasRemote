...
const mouseX = ref(0)
const mouseY = ref(0)
const statusColor = computed(() => { const s = appStore.connectionStatus; return s === 'connected' ? 'online' : s === 'connecting' ? 'connecting' : s === 'error' ? 'error' : 'offline' })
const statusText = computed(() => { const s = appStore.connectionStatus; return s === 'connected' ? 'Connected' : s === 'connecting' ? 'Connecting...' : s === 'error' ? 'Failed' : 'Disconnected' })
